...
void setup() {
  ...
  Serial.println("  Servos: w (cima), s (baixo), a (esq), d (dir)");
  Serial.println("  Movimento: f (frente), t (tras), p (parar)");
  Serial.println("  Exemplos: 'f', 'fe30', 'fd45', 't', 'p'");
  Serial.println("  Acessorios: l (lanterna), m (medir)");
  Serial.println();
}
...
void loop() {
  ...
  // LOOP PARTE 2: MOTORES E COMANDOS SERIAL
  ...
  if (Serial.available() > 0) {
    ...
    // SERVOS - WASD
    else if (texto == "w" && pos1 < 180){
      pos1++;
      servo1.write(pos1);
      delay(30);
      Serial.println(pos1);
    } 
    else if (texto == "s" && pos1 > 0){
      pos1--;
      servo1.write(pos1);
      delay(30);
      Serial.println(pos1);
    }
    else if (texto == "a" && pos2 < 180){
      pos2++;
      servo2.write(pos2);
      delay(30);
      Serial.println(pos2);
    }
    else if (texto == "d" && pos2 > 0){
      pos2--;
      servo2.write(pos2);
      delay(30);
      Serial.println(pos2);
